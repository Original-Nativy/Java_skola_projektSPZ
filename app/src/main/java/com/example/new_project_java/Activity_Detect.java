...
public class Activity_Detect extends CameraActivity {
    ...
    private CameraBridgeViewBase.CvCameraViewListener2 cvCameraViewListener = new CameraBridgeViewBase.CvCameraViewListener2() {
            ...
            if(dict.size()<6)
            {
                ...
                for (int i = 0; i<item.size() ; i++) //item.size()
                {
                    ...
                    if (Math.abs(arD1.get(i).second - klic) > (arD1.get(i)).second * 0.4) {
                        ...
                        item = arD1.subList(0, (int) --minipocitadlo);//error
                        Log.d("sirka", ": em" );
        ...
        public Mat spzPismenaUprava(List<Pair<Integer, Double>> zalozni, Mat _letterimage, ArrayList<MatOfPoint> contours) {
            ...
            for (int i = 0; i < zalozni.size(); i++) {
                ...
                Log.d("Test", "imgout:" +letterimage.width()+" vyska "+ letterimage.height());
                Log.d("Test", "imgout:" +rec.width+" vyska "+ rec.height+"  " +rec.x+" "+rec.y);
                //if(rec.x + rec.width-1 < letterimage.width() && rec.y + rec.height-1 < letterimage.height())
                letterimage= letterimage.submat(rec.x+1 , rec.x+rec.width, rec.y+1, rec.y+rec.height);
                Log.d("Test", "ahoj:" );

                Pomoc=Pomoc2;
                Pomoc= Pomoc.submat(rec.x+1,rec.x+rec.width,1,rec.height+1);
            ...
            Bitmap obraz = Bitmap.createBitmap(imgoutHelp.width(),imgoutHelp.height(), Bitmap.Config.ARGB_8888);
            Utils.matToBitmap(imgoutHelp, obraz);
            MediaStore.Images.Media.insertImage(getContentResolver(), obraz, "whatever" , "pipil");
            Log.d("Test", "sda:");
            ...
            if(i%10 ==0) {//co x snimku aplikuje vyhledavac spz
                for(int i = 0; i<2; i++)
                {
                    Rect plateLoc = i !=1 ? GetPlateLocation(sourceImage,1,2,8)
                            : GetPlateLocation(sourceImage,1,2,0) ;
                    obdelnik= plateLoc;
